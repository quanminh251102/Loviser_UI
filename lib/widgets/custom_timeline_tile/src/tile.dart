...
class TimelineTile extends StatelessWidget {
  const TimelineTile({
    ...
    this.isFirst = false,
    this.isLast = false,
    this.indicatorStyle = const IndicatorStyleCustom(width: 25),
    this.beforeLineStyle = const LineStyleCustom(),
    LineStyleCustom? afterLineStyle,
...
class _TimelinePainter extends CustomPainter {
  _TimelinePainter({
    ...
    this.isFirst = false,
    this.isLast = false,
    required IndicatorStyleCustom indicatorStyle,
    required LineStyleCustom beforeLineStyle,
    required LineStyleCustom afterLineStyle,
  ...
  void _drawSingleLine(
    ...
    if (!isFirst) {
      ...
      if (!(beforeLinePaint.color == Color(0xffec1c24))) {
        var distance = (beginTopLine.dx - endTopLine.dx).abs() +
            (beginTopLine.dy - endTopLine.dy).abs();
        distance = distance * 0.2;

        for (int i = 0; i < 5; i = i + 2) {
          Offset begin = ui.Offset(
            beginTopLine.dx,
            beginTopLine.dy + distance * i,
          );
          Offset end = ui.Offset(
            beginTopLine.dx,
            beginTopLine.dy + distance * (i + 1),
          );
          canvas.drawLine(begin, end, afterLinePaint);
        }
      } else {
    ...
    if (!isLast) {
      ...
      if (!(afterLinePaint.color == Color(0xffec1c24))) {
        var distance = (beginBottomLine.dx - endBottomLine.dx).abs() +
            (beginBottomLine.dy - endBottomLine.dy).abs();
        distance = distance * 0.2;

        for (int i = 0; i < 5; i = i + 2) {
          Offset begin = ui.Offset(
            beginBottomLine.dx,
            beginBottomLine.dy + distance * i,
          );
          Offset end = ui.Offset(
            beginBottomLine.dx,
            beginBottomLine.dy + distance * (i + 1),
          );
          canvas.drawLine(begin, end, afterLinePaint);
        }
      } else {
